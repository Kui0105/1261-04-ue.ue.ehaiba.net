...
- **话费充值 / 短信群发底栏与确认弹窗（recharge.html / sms.html）**：新增「折扣」统计字段（数据来源 `DB.DISCOUNT = { rate:0.99, label:'9.9折' }`）；确认提交订单弹窗同步展示「折扣 / 优惠金额 / 实付总额（9.9折）」，余额卡片「本次消费金额 / 消费后余额」改用折扣后应付金额。
...
- **代理商中心（已通过状态）全面重构**，按需求规格图重新布局：
  - **页面标题**：改为「代理中心」，说明文案更新为「用户申请升级为代理商，解锁推广链生成、下级（二级分销）管理、佣金明细查看与提现等分佣能力」。
...
2. 确认弹窗底部余额四卡片（账户余额/预授信额度/本次消费金额/消费后余额）由 3 个一排改为 2 个一排（2×2 网格）：recharge.css 与 sms.css 的 .balance-cards 基准与 ≤880 响应式均由 repeat(3,1fr) 改为 1fr 1fr，并移除 ≤640 时末卡跨两列的样式，保持全尺寸 2×2 一致。
...
## 2026-08-13（迭代 44）短信模板去标签 + 充值/短信页统一去个人/企业区分

- data.js：4 个短信模板移除 `category: "营销类"` 字段（前端不再展示分类标签）。
- sms.html：模板列表与已选摘要移除「营销类」badge；删除顶部「个人充值」支付模式 banner；
  确认弹窗/支付执行移除 `sess.type` 个人/企业分支，统一为「余额支付 + 安全验证码」流程（前端不区分个人/企业）。
...
- sms.html：移除页面顶部直接列出的模板，改为「📋 选择短信模板」按钮；点击弹出模板选择弹窗，展示后台配置的可用营销模板（内容 + 单价，只读不可改），客户选择后回填已选摘要卡片（含「更换 ›」入口）
- data.js：SMS_TEMPLATES 全部改为「营销类」，重写文案为营销短信（会员日大促 / 周年庆狂欢 / 节日礼遇 / 专属福利券，均带「回TD退订」）
- style.css：新增 `.sms-tpl-picked` 已选模板摘要卡片样式

---

## 2026-08-06（迭代 33）移除短信模板面板提示文案
...
- **账户中心移除退款说明区域**：删除「退款说明」面板（含「账户余额仅可用于充值消费」「退款需人工沟通办理」「客服联系方式」三段文字），页面仅保留交易明细。
...
- **充值页面增加个人/公司充值模式提示卡片**：在话费充值页面（`recharge.html`）表单顶部新增 `.pay-mode-banner` 支付模式提示区域，根据当前登录用户类型动态展示：
...
- **弹窗滚动后关闭按钮不可点击修复**：订单明细等长内容弹窗，向下滚动后顶部 × 按钮滚出视口无法关闭。
  - `.modal h3` 改为 `position: sticky; top: 0` 吸顶固定，白色背景 + 底部阴影分隔线，负 margin 抵消弹窗 padding 使标题栏贴顶。
...
- **订单管理页移动端表格溢出修复**：8 列表格（订单号/面值/税费/号码数/总额/状态/时间/操作）在手机端右侧被截断。
...
- **图标导航下移 + 移除底部 Tab 栏**：上一轮把图标放进顶栏内联，本轮按需求改到「顶栏正下方」单独一行子导航条（`.subnav`，移动端 `position:sticky;top:60px` 跟随吸顶）。
  ...
  - 移动端仅保留「顶栏下方图标子导航」一处入口，避免与底部 Tab 栏重复。
